...
module.exports.getAllOrders_get = (req, res) => {
  User_Order.find()
    .sort("-createdAt")
    .populate([
      { path: "buyer", select: "_id displayName email phoneNumber" },
      {
        path: "products.product",
        select:
...
module.exports.getAllOrders_get1 = async (req, res) => {
  // let data=await GuestOrder.find();
  // let 
  // res.json({data});
  GuestOrder.find()
    .sort("-createdAt")
...
module.exports.ccavenue_creatOrder_post1 = async (req, res) => {
  let userId = "abc";

  const { products, order_price, coupon_applied, shippingAddress, shippingAddress1 } = req.body;
  // make cart empty
...
module.exports.ccavenueresponsehandler = async (request, response) => {
  ...
  request.on("end", async function () {
    ...
    if (orderData.order_status === "Success") {
      ...
      response
        .writeHead(301, {
          Location: "https://www.aaradhak.com/thank",
        })
        .end();
      ...
      response
        .writeHead(301, {
          Location: "https://www.aaradhak.com/failed",
        })
        .end();
    } else {
      response
...
module.exports.ccavenueresponsehandler1 = async (request, response) => {
  var ccavEncResponse = "",
    ccavResponse = "",
    workingKey = process.env.CC_WORKING_KEY,
    ccavPOST = "";
  ...
  request.on("end", async function () {
    ...
    if (orderData.order_status === "Success") {
      ...
      await GuestOrder.findByIdAndUpdate(orderData.order_id, orderUpdates, {
        new: true,
      })
        .then(async (updatedOrder) => {
          // console.log(updatedOrder, "<<<updated Order");
      ...
      await GuestOrder.findByIdAndDelete(orderData.order_id)
        .then((deletedOrder) => console.log(deletedOrder, "<< Order deleted."))
        .catch((err) => console.log(err));
